...
export async function bumpWorkOrders(workOrders: string[], toDate: Date) {
  ...
  function parseBucketLevel(
    ...
    if (
      !bucketLevel ||
      Array.isArray(bucketLevel) ||
      typeof bucketLevel !== "string"
    )
      return null;

    const [current, total] = bucketLevel?.split("/")?.map(Number) || [0, 60];
    ...
    try {
      ...
      if (error && typeof error === "object" && "response" in error) {
        ...
        if (statusCode === 401) {
          ...
          const message =
            apiErrorMessage ||
            `Access denied for work order ${workOrderId}. You may not have permission to modify this work order.`;
          const error: ErrorResult = {
            type: "api",
            message,
            statusCode,
          ...
          const message =
            apiErrorMessage ||
            `Work order ${workOrderId} not found. It may have been deleted or you may not have access to it.`;
          const error: ErrorResult = {
            type: "api",
            message,
            statusCode,
          ...
          const bucketInfo = parseBucketLevel(
          ...
          const message =
            apiErrorMessage ||
            `Rate limit exceeded for work order ${workOrderId}. Please try again in a few minutes.`;
          const error: ErrorResult = {
            type: "api",
            message,
            statusCode,
          ...
          const message =
            apiErrorMessage ||
            `Lightspeed server error for work order ${workOrderId}. Please try again later.`;
          const error: ErrorResult = {
            type: "api",
            message,
            statusCode,
          ...
          const message =
            apiErrorMessage ||
            `API error (${statusCode}) for work order ${workOrderId}. Please check the work order details.`;
          const error: ErrorResult = {
            type: "api",
            message,
            statusCode,
      ...
      if (error && typeof error === "object" && "code" in error) {
        ...
        if (
          ...
          const message =
            ...
            "Unable to connect to Lightspeed. Please check your internet connection.";
          const error: ErrorResult = {
            type: "network",
            message,
            workOrderId,
